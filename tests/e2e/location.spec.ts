...
test.describe('Location Management', () => {
    test('User can change city and verify persistence', async ({ page }) => {
        ...
        const locationBtn = page.locator('header').getByRole('button').filter({ has: page.locator('.lucide-map-pin') });
        await locationBtn.click();

        // 2. Seleccionar una ciudad diferente (ej: Medellín)
        const medellinOption = page.getByRole('option', { name: /medellín/i }).or(page.getByText(/medellín/i).first());
    ...
    test('Location is sent correctly in search request', async ({ page }) => {
        ...
        const requestPromise = page.waitForRequest(request =>
            request.url().includes('price-scraper') && request.method() === 'POST'
        );
