...
test.describe('AI Assistant (Gemini) Chat', () => {
    test('User can open chat and see response placeholders', async ({ page }) => {
        ...
        const chatButton = page.getByRole('button', { name: /gemini/i }).or(page.getByLabel(/asistente gemini/i)).last();
        await expect(chatButton).toBeVisible();
        await chatButton.click({ force: true });

        // 2. Verificar que se abra el panel lateral o diálogo del chat
        await expect(page.getByText(/asistente inteligente/i).or(page.getByText(/gemini/i))).toBeVisible();

        // 3. Escribir un mensaje
        const input = page.getByPlaceholder(/pregunta sobre los resultados/i).or(page.getByRole('textbox').filter({ hasNot: page.getByPlaceholder(/¿Qué producto deseas auditar hoy?/i) }));
