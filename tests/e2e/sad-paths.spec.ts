...
test.describe('Sad Paths & Error Handling', () => {
    test('Shows "No se encontraron resultados" for invalid products', async ({ page }) => {
        ...
        const emptyState = page.getByText(/no se encontraron resultados/i).or(page.getByText(/intenta con otros términos/i));
        await expect(emptyState).toBeVisible({ timeout: 60000 });
    });

    test('Handles malformed EAN entries in Pareto', async ({ page }) => {
        await page.goto('/');
        await page.locator('nav').getByRole('button', { name: /pareto/i }).click();
        ...
        await expect(page.getByText(/formato inválido/i).or(page.getByText(/sin coincidencia/i))).toBeVisible();
    });
});
