...
test.describe('Mobile Responsiveness', () => {
    test('Header adapts to mobile layout', async ({ page }) => {
        ...
        const menuBtn = page.getByRole('button', { name: /menu/i }).or(page.locator('button').filter({ has: page.locator('svg.lucide-menu') }));
        await expect(menuBtn).toBeVisible({ timeout: 10000 });
        await menuBtn.click();

        // 4. Verificar que las opciones del menú aparezcan (en el Search Dialog)
        await expect(page.getByText('Radar Referencial', { exact: false }).first()).toBeVisible({ timeout: 15000 });
        await expect(page.getByText('Módulo Pareto', { exact: false }).first()).toBeVisible({ timeout: 15000 });
    });
});
