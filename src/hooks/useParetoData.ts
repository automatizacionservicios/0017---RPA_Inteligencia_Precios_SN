...
export const useParetoData = () => {
    ...
    const loadFromGSheet = async (url: string) => {
        ...
        try {
            ...
            toast.error("Error al cargar Google Sheets");
        } finally {
    ...
    return {
        ...
        toggleItemSelection: (id: string) => setItems(prev => prev.map(i => i.id === id ? { ...i, selected: !i.selected } : i)),
        toggleAllItems: (selected: boolean) => setItems(prev => prev.map(i => ({ ...i, selected })))
    };
};
