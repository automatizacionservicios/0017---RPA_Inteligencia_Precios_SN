...
import { Sparkles, TrendingDown, Filter, LayoutGrid, List, Search, Download, ArrowRight, Zap, ListChecks, Loader2 } from "lucide-react";
...
const PromoRadar = () => {
    ...
    const fetchOpportunities = async () => {
        ...
        try {
            ...
            const selectedClusters = [...clusters].sort(() => 0.5 - Math.random()).slice(0, 2).flat();
            ...
            const finalDeals = diverseDeals.sort((a, b) => (b.discountPercentage || 0) - (a.discountPercentage || 0));
                    ...
                    >
                        <div className="max-w-2xl">
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-100 mb-6">
                                ...
                                <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Inteligencia de Descuentos</span>
                            </div>
                            ...
                            <p className="text-stone-500 font-medium text-lg mt-4 max-w-xl">
                                Monitoreo en tiempo real de caídas de precio, ofertas relámpago y diferenciales competitivos en el sector.
                            </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4 text-amber-500" />}
                                Actualizar Radar
                            </Button>
                                ...
                                <AnimatePresence>
                                    {results.length > 0 ? results.map((item, idx) => (
                                            ...
                                            <Card className="h-full overflow-hidden border-none shadow-soft group hover:shadow-2xl transition-all duration-500 rounded-3xl bg-white relative">
                                                ...
                                                <CardHeader className="p-8">
                                                    <div className="flex items-center gap-2 mb-4">
                                                        <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest border-stone-200">
                                                            {item.store}
                                                        </Badge>
                                                        {item.brand && (
                                                            <Badge variant="secondary" className="text-[9px] font-black uppercase tracking-widest bg-stone-100 text-stone-600">
                                                                {item.brand}
                                                            </Badge>
                                                        )}
                                                    </div>
                                                ...
                                                <CardContent className="p-8 pt-0">
                                                    ...
                                                    <div className="space-y-3 p-5 rounded-2xl bg-stone-50 border border-stone-100">
                                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-stone-400">
                                                            ...
                                                            <span className="text-emerald-600">${item.pricePerGram.toFixed(2)}/u</span>
                                                        </div>
                                                        <div className="w-full h-1.5 bg-stone-200 rounded-full overflow-hidden">
                                                            <motion.div
                                                                initial={{ width: 0 }}
                                                                animate={{ width: `${Math.min((item.discountPercentage || 0) + 40, 100)}%` }}
                                                                className="h-full bg-emerald-500"
                                                            />
                                                        </div>
                                                    </div>

                                                    <Button variant="default" asChild className="w-full mt-8 h-12 rounded-xl bg-stone-900 hover:bg-emerald-600 text-white font-black uppercase tracking-widest transition-all gap-2 group/btn">
                                        ...
                                        >
                                            ...
                                            <h3 className="text-xl font-black text-stone-900 uppercase tracking-tighter mb-2">Sin Ofertas Críticas</h3>
                                            <p className="text-stone-400 font-bold uppercase tracking-widest text-[10px] max-w-xs mx-auto">
                                                El radar está patrullando el mercado pero no detectó caídas de precio significativas en este momento.
                                            </p>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
                                <div className="max-w-xl text-center md:text-left">
                                    ...
                                    <p className="text-stone-400 font-medium">
                                        Configura el motor para que te notifique vía Teams o Correo cuando un competidor clave baje sus precios más de un 15%.
                                    </p>
                                </div>
