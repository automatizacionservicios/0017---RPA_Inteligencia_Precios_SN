...
const Benchmark = () => {
  ...
  useEffect(() => {
    const _quickSearch = location.state?.quickSearch;
  ...
  const handleSearch = async (
    ...
    try {
      ...
      if (data.products.length === 0) {
        ...
        const confidenceMsg =
          data.metadata?.confidenceLevel === 'high'
            ? ' (Alta confianza)'
            : data.metadata?.confidenceLevel === 'low'
              ? ' (Baja confianza)'
              : '';
        toast.success(
          `Se encontraron ${data.products.length} producto${data.products.length !== 1 ? 's' : ''}${confidenceMsg}`
        );
      }
    } catch (error) {
        ...
        <div className="max-w-[1440px] mx-auto px-6 xl:px-12 space-y-10">
          ...
          <div className="flex flex-col items-center justify-center gap-6 animate-fade-in">
            <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl border border-stone-100 shadow-sm">
              ...
              >
                <Zap
                  className={`w-3.5 h-3.5 transition-transform duration-500 ${activeTab === 'pareto' ? 'scale-110' : 'scale-100 opacity-60'}`}
                />
              ...
              >
                <StoreIcon
                  className={`w-3.5 h-3.5 transition-transform duration-500 ${activeTab === 'catalog' ? 'scale-110' : 'scale-100 opacity-60'}`}
                />
              ...
              >
                <Search
                  className={`w-3.5 h-3.5 transition-transform duration-500 ${activeTab === 'single' ? 'scale-110' : 'scale-100 opacity-60'}`}
                />
              ...
              <div className="p-1 md:p-2">
                <AnimatePresence mode="wait">
                  {activeTab === 'single' && (
                    ...
                    >
                      <EanSearch onSearch={handleEanSearch} isLoading={isLoading} />
                    </motion.div>
                  )}
            ...
            >
              ...
              <Dialog open={isResultsModalOpen} onOpenChange={setIsResultsModalOpen}>
                <DialogTrigger asChild>
                  <button className="group relative px-10 py-5 bg-stone-900 text-white rounded-2xl font-black uppercase tracking-[0.2em] text-sm shadow-[0_20px_40px_rgba(0,0,0,0.1)] hover:shadow-[0_25px_50px_rgba(16,185,129,0.2)] transition-all duration-500 hover:scale-105 active:scale-95 flex items-center gap-4 overflow-hidden">
                  ...
                  <div className="h-full flex flex-col overflow-hidden">
                    ...
                    <div className="shrink-0 p-8 bg-white border-b border-stone-100 flex items-center justify-center relative">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-xl font-black text-stone-800 uppercase tracking-tighter">
                            Market Intelligence Report
                          </h3>
                          <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest">
                            {results?.searchQuery} • {new Date(results?.timestamp).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    </div>
