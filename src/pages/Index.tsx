...
import { Search, Zap, BarChart3, ExternalLink, ChevronRight, Store, ShoppingBasket, Coffee, Cookie, Beef, IceCream, UtensilsCrossed, ShoppingCart, LineChart, Sparkles, Clock, Construction, ShieldCheck, PackageCheck } from "lucide-react";
import Footer from "@/components/Footer";
import Header from "@/components/Header";
import { Badge } from "@/components/ui/badge";
import { getStoreBrand, STORE_BRANDING } from "@/lib/store-branding";
...
const Home = () => {
  ...
  const handleGlobalSearch = (e?: React.FormEvent) => {
    e?.preventDefault();
    if (searchTerm.trim()) {
      const isEan = /^\d{13}$/.test(searchTerm.trim());
  ...
  const quickAccessCards = [
    {
      ...
      description: "Optimiza tus códigos mediante búsqueda por nombre, marca o categoría con el motor de matching más avanzado.",
    ...
    },
    {
      ...
      description: "Auditoría técnica de precios cruzados y benchmarking competitivo en tiempo real.",
  ...
  const brands = [
    "Colcafé", "Sello Rojo", "Noel", "Tosh", "Pozuelo", "Chiky", "Bokitas",
    "Tru-Blu", "Natu Cereal", "Festival", "Dux", "Jet"
  ];
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
        ...
        <div className="relative max-w-[1440px] mx-auto px-6 xl:px-12 z-10">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 backdrop-blur-md animate-fade-in">
              ...
              <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">SISTEMA DE INTELIGENCIA COMERCIAL v2.0</span>
            </div>
            ...
            <p className="text-lg md:text-xl text-stone-400 max-w-2xl mx-auto font-medium leading-relaxed animate-fade-in delay-200">
              Monitoreo estratégico y auditoría de precios en tiempo real para el ecosistema <span className="text-white font-black italic">Nutresa</span>.
            </p>
            ...
            {[
              ...
              >
                ...
                <div className="text-center">
                  <h5 className="text-[13px] font-black uppercase tracking-widest text-stone-900 mb-2">{step.title}</h5>
      ...
      <section className="max-w-[1440px] mx-auto px-6 xl:px-12 py-24 relative z-20 -mt-24">
        <div className="flex flex-col mb-12 animate-fade-in text-center md:text-left">
          ...
          >
            ...
            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em]">Ecosistema Profesional</span>
          </motion.div>
          <motion.h2
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 auto-rows-[240px]">
            ...
            >
              ...
              <div className="flex items-center gap-8 relative z-10">
                ...
                <div>
                  <h3 className="text-2xl font-black text-white uppercase tracking-tight mb-1">Carga Masiva (Pareto)</h3>
                  <p className="text-stone-400 text-xs font-medium">{quickAccessCards[1].description}</p>
                </div>
              </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-xl font-black text-stone-900 uppercase tracking-tight">{quickAccessCards[2].title}</h3>
                  <div className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse" />
                </div>
                <p className="text-stone-500 text-sm font-medium pr-12">{quickAccessCards[2].description}</p>
              </div>
            </motion.div>
            ...
            <div className="lg:col-span-7 lg:row-span-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {[quickAccessCards[4], quickAccessCards[3], quickAccessCards[5], quickAccessCards[6]].map((card, idx) => (
                <motion.div
                  key={idx}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  {card.id === 'oportunidades' && (
                    <div className="absolute top-3 right-5">
                      <div className="flex items-center gap-1.5 px-2 py-0.5 bg-rose-50 rounded-full border border-rose-100">
                        ...
                        <span className="text-[7px] font-black text-rose-500 uppercase tracking-widest">Live</span>
                      </div>
                    </div>
                  )}
                  <div className={`w-12 h-12 rounded-2xl bg-stone-50 flex items-center justify-center mb-4 group-hover:scale-110 ${card.id === 'oportunidades' ? 'group-hover:bg-amber-50' : 'group-hover:bg-emerald-50'} transition-all`}>
                    <card.icon className={`w-6 h-6 ${card.id === 'oportunidades' ? 'text-amber-500' : 'text-stone-800'} group-hover:text-emerald-600`} />
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-stone-900 uppercase tracking-tight mb-1">{card.title}</h4>
                    <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{card.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section className="max-w-[1700px] mx-auto px-6 xl:px-12 py-24 bg-stone-50/50">
        <div className="w-full">
          <div className="text-center mb-16 space-y-3">
            <h3 className="text-4xl font-black text-stone-900 uppercase tracking-tighter">Fuentes Monitorizadas</h3>
            <p className="text-stone-400 font-bold text-[12px] uppercase tracking-[0.3em] opacity-80">Retail Intelligence Network Colombia</p>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
            {Object.entries(STORE_BRANDING).filter(([_, brand]) => brand.name !== 'Nutresa' && brand.icon).map(([id, brand], idx) => {
              const effectiveStatus = storeHealth.getEffectiveStatus(id, brand.status || 'online');
              ...
              return (
                ...
                >
                  ...
                  <div
                    className="absolute -inset-2 opacity-0 group-hover:opacity-10 transition-opacity duration-500 blur-2xl bg-[var(--brand-color)]"
                  />
                  ...
                  <div className="text-center relative z-10 w-full px-2">
                    <span className="block text-[12px] font-black text-stone-800 uppercase tracking-tight mb-1 group-hover:text-emerald-700 transition-colors">{brand.name}</span>
                    <div className="flex flex-col gap-1 items-center">
                      {effectiveStatus === 'maintenance' ? (
                        ...
                        <span className="text-[9px] font-bold text-stone-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">Ver Catálogo</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div
                    className="absolute bottom-0 left-0 h-1 w-0 group-hover:w-full transition-all duration-700 ease-out bg-[var(--brand-color)]"
                  />
                </motion.div>
              );
            })}
      ...
      <section className="bg-white border-y border-stone-100">
        <div className="max-w-[1700px] mx-auto px-6 xl:px-12 py-24">
          <div className="text-center mb-20 space-y-3">
            <h3 className="text-4xl font-black text-stone-900 uppercase tracking-tighter leading-none">Categorías Estratégicas</h3>
            <p className="text-stone-400 font-bold text-[12px] uppercase tracking-[0.3em]">Verticales de Consumo Grupo Nutresa</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 w-full">
            {[
              ...
              >
                ...
                <h4 className="font-black text-stone-800 text-sm uppercase tracking-tight mb-2">{cat.name}</h4>
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{cat.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-[1700px] mx-auto px-6 xl:px-12 py-24">
        <div className="text-center mb-16">
          <h3 className="text-4xl font-black text-stone-900 uppercase tracking-tighter mb-4">Ecosistema Nutresa</h3>
          <p className="text-stone-400 font-bold text-[12px] uppercase tracking-[0.3em] opacity-80">Conexiones Estratégicas y Herramientas</p>
        </div>
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-10 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em]">Recurso Externo</span>
                </div>
