...
import { Search, PackageCheck, AlertCircle, ShoppingCart, Store, ChevronRight, Info, CheckCircle2, XCircle, Clock } from "lucide-react";
import { toast } from "sonner";
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import LoadingProgress from "@/components/LoadingProgress";
import BenchmarkSearch, { Store as StoreType } from "@/components/BenchmarkSearch";
                ...
                <div className="space-y-12">
                    ...
                    <div className="text-center space-y-4 max-w-2xl mx-auto">
                        ...
                        >
                            ...
                            <span className="text-[10px] font-black text-amber-600 uppercase tracking-[0.2em]">Módulo de Disponibilidad v1.0</span>
                        </motion.div>
                        ...
                        <p className="text-stone-500 text-sm font-bold uppercase tracking-widest leading-relaxed">
                            Verifica el stock real en toda la red retail. Evita agotamientos y optimiza el despacho.
                        </p>
                    </div>
                        ...
                        >
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-2xl font-black text-stone-900 uppercase tracking-tight">Estado de Inventario</h3>
                                        <p className="text-[11px] font-bold text-stone-400 uppercase tracking-widest">Resultados para: {searchTerm}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {results.products?.map((product: any, idx: number) => {
                                    ...
                                    const isAvailable = product.price > 0 && !product.productName.toLowerCase().includes('agotado');
                                                ...
                                                <CardContent className="p-6 space-y-6">
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <span className="text-[10px] font-black text-stone-800 uppercase tracking-widest">{product.store}</span>
                                                        </div>
                                                        <Badge className={`rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-widest ${isAvailable
                                                            ? 'bg-emerald-50 text-emerald-600 border-emerald-100'
                                                            : 'bg-rose-50 text-rose-600 border-rose-100'
                                                            }`}>
                                                            {isAvailable ? (
                                                                <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3" /> En Stock</span>
                                                            ) : (
                                                                <span className="flex items-center gap-1.5"><XCircle className="w-3 h-3" /> Agotado</span>
                                                            )}
                                                        </Badge>
                                                    </div>
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{product.brand || 'Marca no detectada'}</p>
                                                    </div>

                                                    <div className="pt-4 border-t border-stone-50 flex items-center justify-between">
                                                        <div className="flex flex-col">
                                                            <span className="text-[9px] font-black text-stone-400 uppercase tracking-[0.2em] mb-1">Último Precio</span>
                                                            <span className="text-lg font-black text-stone-900 tracking-tighter">${product.price?.toLocaleString()}</span>
                                                        </div>
                                                        <Button
                                                            variant="ghost"
                    ...
                    {!isLoading && !results && (
                        ...
                        >
                            ...
                            <div className="space-y-4 max-w-lg mx-auto px-6">
                                <h4 className="text-2xl font-black text-stone-900 uppercase tracking-tighter leading-none">Inicia una <span className="text-amber-600 italic">Auditoría Global</span></h4>
                                <p className="text-stone-400 text-xs font-bold uppercase tracking-widest leading-loose">
                                    Ingresa un producto para verificar su disponibilidad inmediata en la red de aliados y tiendas propias Nutresa.
                                </p>
                            </div>
                        </motion.div>
                    )}
