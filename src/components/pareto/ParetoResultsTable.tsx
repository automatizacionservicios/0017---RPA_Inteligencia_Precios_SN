                                        ...
                                        {item.foundProducts && item.foundProducts.length > 0 ? (
                                            <div className="flex justify-end p-2">
                                                <div className="flex -space-x-3 transition-all duration-300">
                                                    {Array.from(new Set(item.foundProducts.map(p => p.store))).slice(0, 5).map((storeId, sIdx) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {brand.icon ? (
                                                                    ...
                                                                    <span className="text-[8px] font-black text-stone-400">{storeId.substring(0, 2).toUpperCase()}</span>
                                                                )}
                                                            </motion.div>
                                                        );
                                                    })}
                                                    {new Set(item.foundProducts.map(p => p.store)).size > 5 && (
                                                        ...
                                                        >
                                                            <span className="text-[10px] font-black text-emerald-600">+{new Set(item.foundProducts.map(p => p.store)).size - 5}</span>
                                                        </motion.div>
                                                    )}
                                                </div>
                                            </div>
                                        ) : item.status === 'completed' ? (
                                            <span className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Sin Hallazgos</span>
                                        ) : null}
                                    </TableCell>
                                </motion.tr>
                            ))}
