  ...
  return (
    <div className="p-4 md:p-6 lg:p-10 animate-fade-in">
      <AnimatePresence mode="wait">
        {isAuditing && !isFinished ? (
          <motion.div
            key="collapsed-pareto"
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: 'auto' }}
            exit={{ opacity: 0, height: 0 }}
            className="max-w-6xl mx-auto mb-8"
          >
            <div className="bg-stone-900 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6 border border-white/10 shadow-2xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => resetAudit()}
                  className="h-10 rounded-xl border-white/10 bg-white/5 text-white font-bold uppercase tracking-widest hover:bg-rose-500/20 hover:text-rose-400 transition-all text-[9px]"
                >
                  Detener
                </Button>
              </div>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="bg-stone-900 text-white rounded-xl p-3 border-none shadow-xl">
                              <p className="text-[10px] font-bold">
                                Extrae solo el precio más bajo entre todas las tiendas.
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    <CollapsibleTrigger asChild>
                      <Button
                        variant="outline"
                        className="w-full h-14 rounded-2xl border-stone-200 bg-white font-bold text-stone-600 shadow-sm hover:bg-stone-50 hover:border-emerald-200 hover:text-emerald-700 transition-all text-xs uppercase tracking-wide"
                      >
                        <div
                          className={`p-1.5 rounded-lg mr-3 transition-colors ${showStoreConfig ? 'bg-emerald-100 text-emerald-600' : 'bg-stone-100 text-stone-400'}`}
                        >
                          <StoreIcon className="w-4 h-4" />
                        </div>
                        Tiendas ({enabledStoresCount})
                        <ChevronDown
                          className={`ml-auto w-4 h-4 transition-transform duration-300 text-stone-400 ${showStoreConfig ? 'rotate-180 text-emerald-500' : ''}`}
                        />
                      </Button>
                    </CollapsibleTrigger>
                  </Collapsible>
