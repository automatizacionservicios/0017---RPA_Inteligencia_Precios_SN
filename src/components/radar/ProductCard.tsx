            ...
            <Card className="group relative overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-500 bg-white rounded-3xl">
                ...
                <CardContent className="p-8 flex flex-col items-center text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <h5 className="text-lg font-black text-stone-800 uppercase tracking-tighter leading-none">{product.store}</h5>
                        <p className="text-[11px] font-bold text-stone-400 uppercase tracking-widest leading-relaxed">
                            Infraestructura protegida.<br />Se requiere consulta manual.
                        </p>
                    </div>
                    <Button
    ...
    return (
        <Card className="group relative overflow-hidden border-none shadow-xl hover:shadow-2xl transition-all duration-500 bg-white rounded-3xl">
            <div className="absolute top-0 right-0 p-4 z-10">
                <Badge className={`border-none px-3 py-1 text-[9px] font-black uppercase tracking-widest ${product.availability === 'Disponible' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
            ...
            <CardContent className="p-0">
                <div className="relative h-48 bg-stone-50 p-6 flex items-center justify-center group-hover:bg-stone-100 transition-colors duration-500">
                    ...
                    <img src={brand.icon} alt={product.store} className="w-24 h-24 object-contain opacity-80 group-hover:scale-110 transition-transform duration-500 mix-blend-multiply" />
                </div>
                <div className="p-6 space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-stone-400">{product.store}</span>
                    </div>

                    <h3 className="font-bold text-stone-800 leading-tight text-sm line-clamp-2 min-h-[2.5rem]" title={product.productName}>
                        {product.productName}
                    </h3>

                    <div className="flex items-end justify-between pt-2 border-t border-stone-100">
                        <div className="space-y-1">
                            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">Precio</p>
                        ...
                        <div className="text-right space-y-1">
                            {product.gramsAmount > 0 && (
                                <>
                                    <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wider">x Gramo</p>
