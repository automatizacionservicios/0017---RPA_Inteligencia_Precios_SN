...
export const GeminiChatInterface = ({ products, metadata, searchQuery }: GeminiChatInterfaceProps) => {
                ...
                <div className="space-y-8 pb-6">
                    <AnimatePresence initial={false}>
                        {messages.map((msg, idx) => (
                            ...
                            >
                                <div className={`w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm border transition-transform hover:scale-105 ${msg.role === "user"
                                ...
                                <div className={`flex flex-col max-w-[80%] ${msg.role === "user" ? "items-end text-right" : "items-start"}`}>
                                    <div className={`p-4 rounded-3xl text-[13px] font-medium leading-relaxed shadow-sm transition-all ${msg.role === "user"
                    ...
                    {isLoading && (
                        ...
                        >
                            <div className="w-9 h-9 rounded-2xl bg-white border border-stone-100 shadow-sm flex items-center justify-center p-2">
                                <img src="/gemini-icon.png" alt="Gemini" className="w-full h-full object-contain animate-pulse" />
                            </div>
                            <div className="bg-white/50 backdrop-blur-sm border border-stone-100 px-5 py-4 rounded-3xl rounded-tl-none shadow-sm">
                                <div className="flex gap-1.5">
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/40 animate-bounce" style={{ animationDelay: "0ms" }} />
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/40 animate-bounce" style={{ animationDelay: "150ms" }} />
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/40 animate-bounce" style={{ animationDelay: "300ms" }} />
                                </div>
                            </div>
                        </motion.div>
                    )}
