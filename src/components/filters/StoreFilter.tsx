        ...
        {safeStores.map(store => {
          ...
          return (
            <div
              ...
              className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all duration-300 cursor-pointer group select-none ${isSelected
                ? 'bg-emerald-50 border-emerald-100 shadow-sm'
                : 'bg-white border-stone-100 hover:border-stone-200'
                }`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full overflow-hidden border flex items-center justify-center bg-white shadow-sm transition-all ${isSelected ? 'border-emerald-200' : 'border-stone-100'
                  }`}>
                  {brand.icon ? (
                    <img src={brand.icon} alt={brand.name || store} className="w-full h-full object-contain" />
                  ) : (
