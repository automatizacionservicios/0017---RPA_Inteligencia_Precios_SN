...
import { ExternalLink, ArrowUpDown, Download, AlertTriangle, XCircle, Barcode, Info, ChevronDown, ChevronRight, Zap, ListChecks, History, LayoutGrid, Store as StoreIcon, TrendingDown } from "lucide-react";
...
const BenchmarkResults = ({ products, rawContent, citations, metadata, searchQuery, timestamp, isEanSearch }: BenchmarkResultsProps) => {
  ...
  const groupedProducts = useMemo(() => {
    ...
    const STOP_WORDS = new Set(['cafe', 'tostado', 'molido', 'fuerte', 'tradicional', 'bolsa', 'crema', 'organico', 'balanceado', 'soluble', 'descafeinado', 'especial', 'doy', 'pack', 'tetra']);
    ...
    filteredProducts.forEach(product => {
      if (!product.ean) {
        ...
        let matchKey = Object.keys(groups).find(key => {
          return groups[key].some(p => {
            ...
            if (weight && pWeight && Math.abs(weight - pWeight) > Math.max(weight, pWeight) * 0.05) return false;
            ...
            return common.length >= 2 && common.length / minTokens >= 0.6;
          });
        });
  ...
  if (products.length === 0) {
    return (
      <Card className="border-none bg-white shadow-soft rounded-3xl overflow-hidden p-12 text-center group">
        ...
        <p className="text-stone-400 font-extrabold uppercase tracking-widest text-[11px]">No se encontraron registros</p>
        <p className="text-stone-300 text-sm mt-1">Refina los parámetros de búsqueda o cambia los filtros</p>
      </Card>
    );
  }
    ...
    <div className="space-y-10">
      ...
      >
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
          ...
          <div>
            <h5 className="text-[10px] font-black uppercase tracking-widest text-stone-800">Optimal (Líder)</h5>
            <p className="text-[10px] text-stone-400 font-bold mt-1">El precio más bajo con un ahorro significativo frente al resto.</p>
          </div>
        </div>
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
          ...
          <div>
            <h5 className="text-[10px] font-black uppercase tracking-widest text-stone-800">Alerta Gramaje</h5>
            <p className="text-[10px] text-stone-400 font-bold mt-1">Detectamos que este producto tiene un peso diferente al promedio.</p>
          </div>
        </div>
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-white border border-stone-100 shadow-sm">
          ...
          <div>
            <h5 className="text-[10px] font-black uppercase tracking-widest text-stone-800">Match por EAN</h5>
            <p className="text-[10px] text-stone-400 font-bold mt-1">Agrupado automáticamente por código de barras de 13 dígitos.</p>
          </div>
        </div>
      </motion.div>
        ...
        <CardHeader className="p-8 pb-6 bg-stone-50/50 border-b border-stone-100/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <CardTitle className="text-xl font-black text-stone-800 uppercase tracking-tight leading-tight">Malla de Resultados</CardTitle>
                <CardDescription className="text-xs font-bold text-stone-400 uppercase tracking-widest mt-0.5">
                  ...
                  {isEanSearch && <span className="ml-2 text-emerald-600 border-l border-stone-200 pl-2">Modo Localizador de EAN</span>}
                </CardDescription>
              </div>
            </div>
            <Button
            ...
            <Table>
              <TableHeader className="bg-stone-50/30">
                <TableRow className="hover:bg-transparent border-stone-100">
                  ...
                  <TableHead className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Descripción Técnica</TableHead>
                  <TableHead
                  ...
                  <TableHead className="text-[10px] font-black text-stone-400 uppercase tracking-widest w-[140px]">Estado</TableHead>
                  <TableHead className="text-right pr-8 w-[60px]"></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {sortedGroups.map((group, idx) => {
                  const validProducts = group.products.filter(p => p.price > 0 || p.isExternalLink);
                  const hasCompetition = validProducts.length > 1;

                  // Calcular el Gramaje Objetivo del grupo (el más frecuente o el mayor/base)
                  const weights = validProducts.map(p => p.gramsAmount || getWeight(p.productName)).filter(w => w !== null) as number[];
                  const groupTargetWeight = weights.length > 0 ? (
                    // Usar la moda o el valor que más se repite
                    Object.entries(weights.reduce((acc, w) => ({ ...acc, [w]: (acc[w] || 0) + 1 }), {} as Record<number, number>))
                  ...
                  const uniquePrices = Array.from(new Set(benchmarkProducts.map(p => p.price))).sort((a, b) => a - b);
                  ...
                  const savingsVsNext = (lowestPriceForGroup > 0 && nextBestPrice > 0) ? nextBestPrice - lowestPriceForGroup : 0;
                    ...
                    <React.Fragment key={group.key}>
                      ...
                      >
                        <TableCell colSpan={6} className="py-4 px-8">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              <div className={`p-2 rounded-xl border transition-all duration-500 ${isCollapsed ? 'bg-white border-stone-100' : 'bg-emerald-600 border-emerald-500 shadow-md shadow-emerald-200'}`}>
                                <Barcode className={`w-4 h-4 transition-colors ${isCollapsed ? 'text-stone-400' : 'text-white'}`} />
                              </div>
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  <span className={`text-[10px] font-mono font-black border px-2 py-0.5 rounded-lg transition-all ${isEanSearch ? 'bg-emerald-600 text-white border-emerald-500 scale-110 shadow-md ring-4 ring-emerald-500/10' : 'text-stone-800 bg-stone-100 border-stone-200'}`}>
                                    {group.ean || "S/EAN"}
                                  </span>
                            ...
                            <div className="flex items-center gap-3">
                              {isCollapsed && (
                                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-stone-100">
                                  <span className="text-[9px] font-black text-stone-400 uppercase tracking-widest">Min:</span>
                                  <span className="text-[11px] font-black text-emerald-600">${group.minPrice.toLocaleString('es-CO')}</span>
                                </div>
                              )}
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center border border-stone-200 bg-white transition-transform duration-500 ${!isCollapsed ? 'rotate-180' : ''}`}>
                  ...
                  {rawContent && (
                    <div className="flex-1 min-w-[300px]">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Análisis de Profundidad (Perplexity)</span>
                      </div>
                  ...
                  {citations && citations.length > 0 && (
                    <div className="w-[300px] flex-shrink-0">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-[10px] font-black text-stone-500 uppercase tracking-widest">Fuentes Verificadas</span>
                      </div>
                      <div className="flex flex-col gap-2">
                        {citations.map((citation, index) => (
                          ...
                          >
                            ...
                            <span className="text-[10px] text-stone-400 group-hover:text-stone-700 truncate font-medium">{citation}</span>
                          </a>
                        ))}
                      </div>
                    </div>
      ...
      {
        metadata?.validation && metadata.validation.rejected > 0 && metadata.validation.rejectedProducts && (
          <Card className="border-none bg-rose-50/30 shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-3xl overflow-hidden p-8">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-black text-stone-800 uppercase tracking-tight">Filtros de Validación Activos</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {metadata.validation.rejectedProducts.map((rejected, index) => (
                <div key={index} className="p-4 rounded-2xl bg-white border border-rose-100 group transition-all duration-500 hover:shadow-xl hover:shadow-rose-100/20">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[9px] font-black text-rose-400 uppercase tracking-widest">{rejected.store}</span>
                  ...
                  <p className="text-xs font-black text-stone-800 truncate uppercase mt-1">{rejected.productName}</p>
            ...
            <div className="mt-8 p-4 bg-white/50 rounded-2xl border border-stone-100 text-center">
              <p className="text-[10px] font-black text-stone-300 uppercase tracking-widest">Optimización de Precisión IA habilitada</p>
            </div>
          </Card>
        )
      }
