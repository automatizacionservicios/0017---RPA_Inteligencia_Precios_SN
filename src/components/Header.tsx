...
import { Search, Zap, Command, LayoutDashboard, Store as StoreIcon, FileText, Settings, Sparkles, MessageSquare, PackageCheck } from "lucide-react";
...
const Header = () => {
  ...
  const navItems = [
    ...
    { title: "Radar Intelligence", href: "/benchmark", state: { mode: 'single' }, icon: Search, category: "Herramientas" },
    { title: "Oportunidades", href: "/oportunidades", icon: Sparkles, category: "Herramientas" },
    { title: "Módulo Pareto", href: "/benchmark", state: { mode: 'pareto' }, icon: Zap, category: "Herramientas" },
    { title: "Radar Referencial", href: "/radar-referencial", icon: StoreIcon, category: "Herramientas" },
    { title: "Auditoría Stock", href: "/auditoria-stock", icon: PackageCheck, category: "Herramientas" },
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 w-full transition-all duration-700 ${isHomePage
        ? 'bg-stone-900/60 border-b border-white/5 backdrop-blur-2xl'
        : 'bg-white/90 border-b border-stone-200 backdrop-blur-xl'
        }`}>
        {/* Subtle top glow line */}
        <div className={`absolute top-0 left-0 right-0 h-[1px] ${isHomePage ? 'bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent' : 'bg-transparent'}`} />

        <div className="max-w-[1440px] mx-auto px-6 xl:px-12">
          <div className="flex h-16 items-center justify-between">
            <Link to="/" className="flex items-center gap-5 group">
              <div className="relative">
                <div className={`absolute -inset-2 bg-emerald-500/20 rounded-full blur-xl transition-opacity duration-700 opacity-0 group-hover:opacity-100`} />
                <img
              ...
              <div className="flex flex-col">
                <span className={`text-[13px] font-black uppercase tracking-[0.3em] leading-none ${isHomePage ? 'text-white' : 'text-stone-900'}`}>Radar</span>
                <span className={`text-[9px] font-black uppercase tracking-widest leading-none mt-1.5 opacity-60 ${isHomePage ? 'text-stone-400' : 'text-stone-500'}`}>Market Intel</span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-8">
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase tracking-widest pr-4">Buscar módulo...</span>
                <kbd className={`pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border px-1.5 font-mono text-[10px] font-medium opacity-100 ${isHomePage ? 'bg-white/10 border-white/10 text-white/50' : 'bg-white border-stone-200 text-stone-400'}`}>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className={`hidden md:flex items-center gap-3 px-4 py-2.5 rounded-full border backdrop-blur-sm transition-all duration-700 ${isHomePage
                  ...
                  <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${isHomePage ? 'text-emerald-400' : 'text-stone-600'}`}>
          ...
          <CommandList className="max-h-[450px] p-2 custom-scrollbar">
            <CommandEmpty className="py-10 text-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm font-bold text-stone-400 uppercase tracking-widest">No se encontraron resultados</p>
              </div>
            </CommandEmpty>
            ...
            {["Herramientas", "Navegación", "Soporte"].map((cat) => (
              <CommandGroup
                key={cat}
                heading={<span className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 mb-2">{cat}</span>}
              >
                {navItems.filter(item => item.category === cat).map((item) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-stone-800 uppercase tracking-tight group-hover:text-emerald-700">{item.title}</span>
                      <span className="text-[10px] font-medium text-stone-400 group-hover:text-emerald-500/70">Navegar al módulo de {item.title.toLowerCase()}</span>
                    </div>
                  </CommandItem>
                ))}
              </CommandGroup>
