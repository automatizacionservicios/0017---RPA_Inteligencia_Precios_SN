...
export class VtexStrategy implements ISearchStrategy {
  ...
  private async searchLegacy(
    ...
    try {
      ...
      const basePath = isExitoGroup
        ? '/io/api/catalog_system/pub/products/search'
      ...
      return data
        .map((p: any) => {
          try {
            ...
            if (!item && !isEan) {
              item = p.items?.find(
                (i: any) => (i.sellers?.[0]?.commertialOffer?.AvailableQuantity || 0) > 0
              );
            }
    ...
    try {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.warn(
            `[VTEX-IO] ${this.storeName} falló con estado ${res.status}. ${errorText.substring(0, 100)}`
          );
          return [];
        }
