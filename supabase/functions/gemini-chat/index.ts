...
serve(async (req) => {
    ...
    try {
        ...
        if (!GEMINI_API_KEY) {
            ...
            return new Response(
                JSON.stringify({ message: "Error de configuración: API Key no encontrada. Por favor contacta al administrador." }),
        ...
        if (!products || !Array.isArray(products) || products.length === 0) {
            ...
            return new Response(
                JSON.stringify({ message: "No veo productos en tu consulta actual. Por favor realiza una búsqueda primero para que pueda analizar los datos." }),
        ...
        const productsTable = products.map((p: any) =>
...
Responde en base ÚNICAMENTE a los datos de la tabla anterior.`;
        ...
        const models = ["gemini-2.0-flash", "gemini-flash-latest", "gemini-1.5-flash", "gemini-pro-latest", "gemini-1.5-pro", "gemini-pro"];
        ...
        for (const model of models) {
            try {
                ...
                aiMessage = data.candidates?.[0]?.content?.parts?.[0]?.text || "No obtuve respuesta del modelo.";
                success = true;
        ...
        if (!success) {
            ...
            throw new Error(`Fallaron todos los modelos (${models.join(', ')}). Verifica los logs de Supabase para ver cuáles tiene permiso tu Key. Último error: ${lastError}`);
        }
